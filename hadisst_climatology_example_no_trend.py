...
for i in range(0, x):
    for j in range(0, y):
        T=(beta_values[i,j,0]+beta_values[i,j,1]*np.sin(2.*np.pi*f*new_t)+beta_values[i,j,2]*np.cos(2.*np.pi*f*new_t)+beta_values[i,j,3]*np.sin(4.*np.pi*f*new_t)+beta_values[i,j,4]*np.cos(4.*np.pi*f*new_t))
